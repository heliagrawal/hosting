...
<div class="offcanvas offcanvas-start menu-offcanvas" tabindex="-1" id="offcanvasMobileMenu">
    <div class="mobile-menu-area">
                ...
                <div class="search-wrap">
                    <select class="form-select">
                        ...
                        @foreach (Category() as $item)
                            <optgroup label="{{ langConverter($item->en_Category_Name, $item->fr_Category_Name) }}">
                                @foreach ($item->subcategory as $itemsubcategory)
                                    <option value="{{ $itemsubcategory->id }}">
                                        {{  langConverter($itemsubcategory->en_SubCategory_Name,$itemsubcategory->fr_SubCategory_Name) }}
                                    </option>
                                @endforeach
                            </optgroup>
                        @endforeach
        ...
        <nav class="main-menu">
            <ul class="menu-list">
                <li class="menu-item"><a class="menu-link" href="{{ route('front') }}">{{ staticMenuName('home') }}</a>
                </li>
                <li class="menu-item">
                ...
                <li class="menu-item"><a class="menu-link" href="{{ route('blog') }}">{{ staticMenuName('blog') }}</a>
                </li>
                <li class="menu-item"><a class="menu-link"
        ...
        <div class="menu-bottom">
            ...
            @if (Auth::user())
                ...
                @endif
                <a class="account-btn mb-3" href="{{ route('user.logout') }}"><i
                        class="user-icon flaticon-user"></i>
                    {{ __('Logout') }}</a>
            @else
